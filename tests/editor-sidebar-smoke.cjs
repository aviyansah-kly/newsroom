...
(async () => {
  ...
  if(!right.rightCollapsed||right.panelWidth>70||right.paperWidth<=before.paperWidth+200||right.writerWidth<=before.writerWidth+200)throw Error('Right sidebar did not collapse and expand the actual writing surface');
  ...
  if(!both.leftCollapsed||both.paperWidth<=right.paperWidth+100||both.writerWidth<=right.writerWidth+100)throw Error('Left sidebar did not expand the actual writing surface');
  await page.locator('#nrRightPanelToggle').click({timeout:10000});
  await page.waitForTimeout(400);
